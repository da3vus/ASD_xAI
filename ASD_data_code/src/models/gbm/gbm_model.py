...
def train_gradient_boosting(X_train, X_test, y_train, y_test, cv_folds=10):
    ...
    explainer = shap.Explainer(best_model.named_steps['gbm'], X_train, feature_names=feature_names)
    shap_values = explainer(X_test)

    # SHAP Summary Plot
    plt.figure()
    shap.summary_plot(shap_values, X_test, show=False)
    plt.title(f"SHAP Summary Plot - GBM ({cv_folds}-Fold CV)")
    plt.show()
    

    #SHAP Waterfall Plot
    plt.title(f"SHAP Waterfall Plot - GBM ({cv_folds}-Fold CV)")
    shap.plots.waterfall(shap_values[0], max_display=10)
    
    #SHAP Bar Plot
    plt.title(f"SHAP Bar Plot - GBM ({cv_folds}-Fold CV)")
...
if __name__ == "__main__":
    try:
        ...
        train_gradient_boosting(X_train, X_test, y_train, y_test, cv_folds=10)
